...
for file_index = 1:3%length(directory) %%%%%%%%%% Change this later
...
figure(2)
imagesc(1:192, 1:steps, rf_data_cell{1})
hold on
for i=1:length(rf_data_cell)
    ...
    if i + (num_trans_elm-1) <= 192
        ...
        % Delay
        for r = 44%:length(roi_r)       % Radial Axis
            for t = 64%length(roi_t)   % Angular Axis
